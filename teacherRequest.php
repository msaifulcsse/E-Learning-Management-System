...
require_once("mysql_db.php");
	...
	$query = mysql_query("SELECT email FROM dummy_teacher WHERE email='$email' UNION SELECT email_id FROM login WHERE email_id='$email';");
	...
	if($nn == 0 && !preg_match('/^ *$/', $fname))
	{
		...
		$sql = "INSERT INTO `dummy_teacher`(`fname`,`occupation`,`gender`,`education`,`address`,`gitlink`,`bio`,`name`,`image`,`email`,`idcreate_date`,`password`) VALUES ('$fname','$occupation','$gender','$education','$address','$url','$bio','$name','$image','$email','$date','$password')";
				if(!mysql_query($sql))
				{
					$msg3= 'Error In Dummy Teacher';
				}
				else
				{
					$msg= 'Registraion Successfull & Thanks For Your Interest!!!';
				}
		}
	}
	else{
		$msg1= 'You Already Send A Request || Registered Teacher With This Email!!!';
	}
